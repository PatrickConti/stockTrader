<!DOCTYPE HTML>
<html>

<head>
    <title>Cirrus - Buttons</title>

    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=0">
    <meta name="description" content="A responsive and lightweight CSS framework.">
    <meta charset="UTF-8">

    <link href="../../dist/cirrus.css" type="text/css" rel="stylesheet" />
    <link href="../../demo_res/css/app.css" type="text/css" rel="stylesheet" />

    <!-- Syntax highlight code -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.11.0/styles/github-gist.min.css">

    <link href="https://fonts.googleapis.com/css?family=Nunito+Sans:200,300,400,600,700" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css?family=Montserrat:400,700" rel="stylesheet">
    <script src="https://code.jquery.com/jquery-2.2.4.min.js" integrity="sha256-BbhdlvQf/xTY9gja0Dq3HiwQF8LaCRTXxZKRutelT44=" crossorigin="anonymous"></script>
</head>

<body>
    <div id="header" class="header header-fill unselectable header-fixed no-transition header-dark">
        <div class="header-brand">
            <div class="nav-item">
                <a href="../../">
                    <h6 class="title">Cirrus</h6>
                </a>
            </div>
            <div class="nav-item nav-btn" id="header-btn">
                <span></span>
                <span></span>
                <span></span>
            </div>
        </div>
        <div class="header-nav" id="header-menu">
            <div class="nav-left">
                <div class="nav-item no-hover">
                    <div class="nav-item">
                        <a href="https://github.com/Spiderpig86/Cirrus">
                        <span class="icon">
                        <i class="fa fa-github"></i>
                        </span>
                        </a>
                    </div>
                    <div class="nav-item">
                        <a href="#">
                        <span class="icon white">
                        <i class="fa fa-twitter"></i>
                        </span>
                        </a>
                    </div>
                </div>
            </div>
            <div class="nav-right nav-menu">
                <div class="nav-item">
                    <a href="https://github.com/Spiderpig86/Cirrus/blob/master/src/button.css" target="_blank"><span class="icon"><i class="fa fa-github"></i></span></a>
                </div>

                <div class="nav-item has-sub">
                    <a class="nav-dropdown-link">Buttons</a>
                    <ul class="dropdown-menu dropdown-animated">
                        <li><a href="#started" class="smoothScroll">Top</a></li>
                        <li><a href="#tooltips" class="smoothScroll">Tooltips</a></li>
                        <li><a href="#groups" class="smoothScroll">Groups</a></li>
                    </ul>
                </div>
            </div>
        </div>
    </div>

    <space class="x-large"></space>

    <section id="started" class="padtop">
        <div class="content">
            <h3><a href="./index.html#started" class="doc-link btn-tooltip tooltip-left" data-tooltip="Click to copy">#</a>Buttons</h3>
            <div class="divider"></div>
            <p>Cirrus provides minimally designed buttons created to work well with any website. It is also minimally designed to provide a blank canvas for designers to craft custom buttons for any design. By default, buttons will be placed next to each other with no space. By wrapping each button with a <code>btn-container</code> div.</p>
            <space></space
            >
             <h6 class="uppercase">Basic options</h6>
                <p class="title">Non-animated buttons.</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn">test</div>
                        </div>
                        <div class="btn-container">
                            <button>Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
                    <pre><code class="html" data-lang="HTML"> &lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;;</code></pre>
                    </div>
                </div>

                <space class="large"></space>

                <p class="title">Animated</ph6>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn btn-animated">test</div>
                        </div>
                        <div class="btn-container">
                            <button class="btn-animated">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" class="btn-animated" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
                    <pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-animated&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-animated&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-animated&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>

            <space class="large"></space>
            <h6 class="uppercase">States</h6>
            <p class="title">Disabled buttons.</p>
            <p class="subtitle">Please note that using <code>&lt;div class=&quot;btn&quot;&gt;test&lt;/div&gt</code> is not supported.</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <button disabled class="btn-animated">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input disabled type="submit" class="btn-animated" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button disabled class=&quot;btn-animated&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input disabled type=&quot;submit&quot; class=&quot;btn-animated&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>

            <p class="title">Disabled accented button.</p>
            <p class="subtitle">Please note that using <code>&lt;div class=&quot;btn&quot;&gt;test&lt;/div&gt</code> is not supported.</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <button disabled class="btn-animated btn-accent">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input disabled type="submit" class="btn-animated btn-accent" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button disabled class=&quot;btn-animated btn-accent&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input disabled type=&quot;submit&quot; class=&quot;btn-animated btn-accent&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>

                <space class="large"></space>

                <h6 class="uppercase">Styles</h6>

                <p class="title">Accent</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn btn-accent">test</div>
                        </div>
                        <div class="btn-container">
                            <button class="btn-accent">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" class="btn-accent" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-accent&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-accent&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-accent&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>

                <space class="large"></space>

                <p class="title">Outline</p>
                <div class="row fluid-container level" style="background: #c7c7c7">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn btn-outline">test</div>
                        </div>
                        <div class="btn-container">
                            <button class="btn-outline">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" class="btn-outline" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-outline&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-outline&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-outline&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>

                <space class="large"></space>

                <p class="title">Inverted Outline</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn btn-outline-inverted">test</div>
                        </div>
                        <div class="btn-container">
                            <button class="btn-outline-inverted">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" class="btn-outline-inverted" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-outline-inverted&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-outline-inverted&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-outline-inverted&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>

                <space class="large"></space>

                <p class="title">Transparent</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn btn-transparent">test</div>
                        </div>
                        <div class="btn-container">
                            <button class="btn-transparent">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" class="btn-transparent" value="Submit"/>
                        </div>

                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML"> &lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-transparent&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-transparent&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-transparent&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>

                <space class="large"></space>

                 <p class="title">Light Gray</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn btn-light">test</div>
                        </div>
                        <div class="btn-container">
                            <button class="btn-light">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" class="btn-light" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-light&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-light&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-light&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>
                
                <space class="large"></space>

                 <p class="title">Dark Gray</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn btn-dark">test</div>
                        </div>
                        <div class="btn-container">
                            <button class="btn-dark">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" class="btn-dark" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-dark&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-dark&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-dark&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>

                <space class="large"></space>

                 <p class="title">Black</p>
                <div class="row fluid-container">
                    <div class="col-fluid">
                        <div class="btn-container">
                            <div class="btn btn-black">test</div>
                        </div>
                        <div class="btn-container">
                            <button class="btn-black">Regular Button</button>
                        </div>
                        <div class="btn-container">
                            <input type="submit" class="btn-black" value="Submit"/>
                        </div>
                    </div>
                    <div class="col-fluid">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-black&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-black&quot;&gt;Regular Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-black&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                    </div>
                </div>


                  <space class="large"></space>
                <h6 class="uppercase">Sizes</h6>
                <p>Besides the regular button size, Cirrus allows you to choose which size buttons you want to use.
                <p class="title">Tiny</p>

                <div class="row">
                    <div class="col-6">
                        <div class="row">
                            <div class="btn-container">
                                <div class="btn btn-tiny">test</div>
                            </div>
                            <div class="btn-container">
                                <button class="btn-tiny">Tiny Button</button>
                            </div>
                            <div class="btn-container">
                                <input type="submit" class="btn-tiny" value="Submit"/>
                            </div>
                        </div>
                        
                    </div>
                    <div class="col-6">
                         <div class="row">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-tiny&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-tiny&quot;&gt;Tiny Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-tiny&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                        </div>
                    </div>
                </div>

                <p class="title">Small</p>
                <div class="row">
                    <div class="col-6">
                        <div class="row">
                            <div class="btn-container">
                                <div class="btn btn-small">test</div>
                            </div>
                            <div class="btn-container">
                                <button class="btn-small">Small Button</button>
                            </div>
                            <div class="btn-container">
                                <input type="submit" class="btn-small" value="Submit"/>
                            </div>
                        </div>
                        
                    </div>
                    <div class="col-6">
                         <div class="row">
            <pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-small&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-small&quot;&gt;Small Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-small&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                        </div>
                    </div>
                </div>

                <p class="title">Large</p>
                <div class="row">
                    <div class="col-6">
                        <div class="row">
                            <div class="btn-container">
                                <div class="btn btn-large">test</div>
                            </div>
                            <div class="btn-container">
                                <button class="btn-large">Large Button</button>
                            </div>
                            <div class="btn-container">
                                <input type="submit" class="btn-large" value="Submit"/>
                            </div>
                        </div>
                    </div>

                    <div class="col-6">
                                                <div class="row">
<pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-large&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-large&quot;&gt;Large Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-large&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre></div>
                    </div>
                </div>

                <p class="title">Extra Large</p>

                <div class="row">
                    <div class="col-6">
                        <div class="row">
                            <div class="btn-container">
                                <div class="btn btn-xlarge">test</div>
                            </div>
                            <div class="btn-container">
                                <button class="btn-xlarge">Extra Large Button</button>
                            </div>
                            <div class="btn-container">
                                <input type="submit" class="btn-xlarge" value="Submit"/>
                            </div>
                        </div>
                        
                    </div>
                    <div class="col-6">
                         <div class="row">
            <pre><code class="html" data-lang="HTML">&lt;div class=&quot;btn-container&quot;&gt;
    &lt;div class=&quot;btn btn-xlarge&quot;&gt;test&lt;/div&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;button class=&quot;btn-xlarge&quot;&gt;Extra Large Button&lt;/button&gt;
&lt;/div&gt;
&lt;div class=&quot;btn-container&quot;&gt;
    &lt;input type=&quot;submit&quot; class=&quot;btn-xlarge&quot; value=&quot;Submit&quot;/&gt;
&lt;/div&gt;</code></pre>
                        </div>
                    </div>
                </div>

                <space class="x-large"></space>
                <h6>Font Awesome</h6>
                <p>Font awesome integration is extremly easy with Cirrus without additional CSS.</p>
                <space></space>

                <p class="title">Glyph on the right.</p>
                <div class="row">
                    <div class="col-6">
                        <a href="#"><button>See More<i class="fa fa-chevron-right pad-left"></i></button></a>
                    </div>
                    <div class="col-6">
<pre><code class="html" data-lang="HTML">&lt;a href=&quot;&quot;&gt;&lt;button&gt;See More&lt;i class=&quot;fa fa-chevron-right pad-left&quot;&gt;&lt;/i&gt;&lt;/button&gt;&lt;/a&gt;</code></pre>
                    </div>
                </div>

                <space></space>

                <p class="title">Glyph on the left.</p>
                <div class="row">
                    <div class="col-6">
                        <a><button><i class="fa fa-chevron-left pad-right"></i>See More</button></a>
                    </div>
                    <div class="col-6">
<pre><code class="html" data-lang="HTML">&lt;a href=&quot;&quot;&gt;&lt;button&gt;&lt;i class=&quot;fa fa-chevron-left pad-right&quot;&gt;&lt;/i&gt;See More&lt;/button&gt;&lt;/a&gt;</code></pre>
                    </div>
                </div>

                <space></space>

                <p class="title">Different glyph sizes.</p>
                <div class="row">
                    <div class="col-6">
                        <a><button class="btn-tiny">Tiny<i class="fa fa-chevron-right pad-left "></i></button></a>
                        <a><button class="btn-small">Small<i class="fa fa-chevron-right pad-left "></i></button></a>
                        <a><button>Normal<i class="fa fa-chevron-right pad-left"></i></button></a>
                        <a><button class="btn-large">Large<i class="fa fa-chevron-right pad-left"></i></button></a>
                        <a><button class="btn-xlarge">Extra Large<i class="fa fa-chevron-right pad-left "></i></button></a>
                        <a><button class="btn-xxlarge">Extra Extra Large<i class="fa fa-chevron-right pad-left "></i></button></a>
                    </div>
                    <div class="col-6">
<pre><code class="html" data-lang="HTML">&lt;a href=&quot;&quot;&gt;&lt;button class=&quot;btn-tiny&quot;&gt;Tiny&lt;i class=&quot;fa fa-chevron-right pad-left &quot;&gt;&lt;/i&gt;&lt;/button&gt;&lt;/a&gt;

&lt;a href=&quot;&quot;&gt;&lt;button class=&quot;btn-small&quot;&gt;Small&lt;i class=&quot;fa fa-chevron-right pad-left &quot;&gt;&lt;/i&gt;&lt;/button&gt;&lt;/a&gt;

&lt;a href=&quot;&quot;&gt;&lt;button&gt;Normal&lt;i class=&quot;fa fa-chevron-right pad-left&quot;&gt;&lt;/i&gt;&lt;/button&gt;&lt;/a&gt;

&lt;a href=&quot;&quot;&gt;&lt;button class=&quot;btn-large&quot;&gt;Large&lt;i class=&quot;fa fa-chevron-right pad-left&quot;&gt;&lt;/i&gt;&lt;/button&gt;&lt;/a&gt;

&lt;a href=&quot;&quot;&gt;&lt;button class=&quot;btn-xlarge&quot;&gt;Extra Large&lt;i class=&quot;fa fa-chevron-right pad-left &quot;&gt;&lt;/i&gt;&lt;/button&gt;&lt;/a&gt;

&lt;a href=&quot;&quot;&gt;&lt;button class=&quot;btn-xxlarge&quot;&gt;Extra Extra Large&lt;i class=&quot;fa fa-chevron-right pad-left &quot;&gt;&lt;/i&gt;&lt;/button&gt;&lt;/a&gt;</code></pre>
                    </div>
                </div>
        </div>
    </section>

    <section id="tooltips">
        <div class="content">
            <h3><a href="./index.html#tooltips" class="doc-link btn-tooltip tooltip-left" data-tooltip="Click to copy">#</a>Button Tooltips</h3>
            <div class="divider"></div>
            <p>Cirrus adds a whole new dimension to buttons with tooltips. Specifying tooltip text requires no extra div but rather just 1 extra property within the buttons themselves allowing for easy customization physically or dynamically with JavaScript.</p>
            <div class="row">
                <div class="col-6 center">
                    <div class="btn-container">
                        <button class="btn-accent btn-tooltip" data-tooltip="Tooltip 1">Button 1</button>
                    </div>
                    <div class="btn-container">
                        <button class="btn-accent btn-tooltip" data-tooltip="Tooltip 2">Button 2</button>
                    </div>
                    <div class="btn-container">
                        <button class="btn-accent btn-tooltip" data-tooltip="Tooltip 3">Button 3</button>
                    </div>
                </div>
                <div class="col-6">
                    <pre><code class="html" data-lang="HTML">
&lt;div class=&quot;col-6 center&quot;&gt;
    &lt;button class=&quot;btn-accent btn-tooltip&quot; data-tooltip=&quot;Tooltip 1&quot;&gt;Button 1&lt;/button&gt;
    &lt;button class=&quot;btn-accent btn-tooltip&quot; data-tooltip=&quot;Tooltip 2&quot;&gt;Button 2&lt;/button&gt;
    &lt;button class=&quot;btn-accent btn-tooltip&quot; data-tooltip=&quot;Tooltip 3&quot;&gt;Button 3&lt;/button&gt;
&lt;/div&gt;
                        </code></pre>
                </div>
            </div>
            <p>The way this works is that the contents of the tooltip are specified by the <code>data-tooltip</code> attribute where the CSS would retrieve the data to be displayed.</p>

            <h6>Tooltip Direction</h6>
            <p>You can also add additional classes to specify which direction the tooltip should display. There are 4 different tooltip directions for now, including <code>tooltip-bottom</code>, <code>tooltip-left</code>, <code>tooltip-right</code>, and the default direction which displays the tooltip at the top. Other secondary directions will be added in  a future update.</p>
            <div class="row">
                <div class="col-6">
                    <div class="btn-container">
                        <button class="btn-tooltip" data-tooltip="Tooltip Top (Default)">Tooltip Top</button>
                    </div>
                    <div class="btn-container">
                        <button class="btn-tooltip tooltip-bottom" data-tooltip="Tooltip Bottom">Tooltip Bottom</button>
                    </div>
                    <div class="btn-container">
                        <button class="btn-tooltip tooltip-left" data-tooltip="Tooltip Left">Tooltip Left</button>
                    </div>
                    <div class="btn-container">
                        <button class="btn-tooltip tooltip-right" data-tooltip="Tooltip Right">Tooltip Right</button>
                    </div>
                </div>
                <div class="col-6">
                    <pre><code class="HTML" data-lang="HTML">&lt;button class=&quot;btn-tooltip&quot; data-tooltip=&quot;Tooltip Top (Default)&quot;&gt;Tooltip Top&lt;/button&gt;
&lt;button class=&quot;btn-tooltip tooltip-bottom&quot; data-tooltip=&quot;Tooltip Bottom&quot;&gt;Tooltip Bottom&lt;/button&gt;
&lt;button class=&quot;btn-tooltip tooltip-left&quot; data-tooltip=&quot;Tooltip Left&quot;&gt;Tooltip Left&lt;/button&gt;
&lt;button class=&quot;btn-tooltip tooltip-right&quot; data-tooltip=&quot;Tooltip Right&quot;&gt;Tooltip Right&lt;/button&gt;</code></pre>
                </div>
            </div>
        </div>
    </section>

    <section id="groups" class="padtop">
        <div class="content">
            <h3><a href="./index.html#groups" class="doc-link btn-tooltip tooltip-left" data-tooltip="Click to copy">#</a>Button Groups</h3>
            <div class="divider"></div>
            <p>Button groups are designed to group buttons of similar actions or properties together in a seemless fashion. Buttons are placed adjacent to each other and buttons at each of the ends are rounded off respectively.</p>
            <div class="row">
                <div class="col-6">
                    <div class="btn-group">
                        <button class="btn-accent">First Button</button>
                        <button class="btn-accent">Second Button</button>
                        <button class="btn-accent">Third Button</button>
                    </div>
                </div>
                <div class="col-6">
                    <pre><code class="HTML" data-lang="HTML"> &lt;div class=&quot;btn-group&quot;&gt;
    &lt;button class=&quot;btn-accent&quot;&gt;First Button&lt;/button&gt;
    &lt;button class=&quot;btn-accent&quot;&gt;Second Button&lt;/button&gt;
    &lt;button class=&quot;btn-accent&quot;&gt;Third Button&lt;/button&gt;
&lt;/div&gt;</code></pre>
                </div>
            </div>
            <p>Just nest the buttons you want to be in a group inside a <code>btn-group</code> container and Cirrus will handle the rest.</p>
            <div class="row">
                <div class="col-6">
                    <div class="btn-group">
                        <button class="">First Button</button>
                        <button class="">Second Button</button>
                        <button class="">Third Button</button>
                    </div>
                </div>
                <div class="col-6">
                    <pre><code class="HTML" data-lang="HTML"> &lt;div class=&quot;btn-group&quot;&gt;
    &lt;button class=&quot;btn-accent&quot;&gt;First Button&lt;/button&gt;
    &lt;button class=&quot;btn-accent&quot;&gt;Second Button&lt;/button&gt;
    &lt;button class=&quot;btn-accent&quot;&gt;Third Button&lt;/button&gt;
&lt;/div&gt;</code></pre>
                </div>
            </div>
            <p>If you want the buttons to flex to fill the width of the container, just add the <code>btn-group-fill</code> class to the <code>btn-group</code> container.</p>
            <div class="row">
                <div class="col-6">
                    <div class="btn-group btn-group-fill">
                        <button class="btn-dark">Look</button>
                        <button class="btn-dark">At</button>
                        <button class="btn-dark">Me</button>
                        <button class="btn-dark">Flex</button>
                    </div>
                    <br>
                    <div class="btn-group btn-group-fill">
                        <button class="">First Button</button>
                        <button class="">Second Button</button>
                    </div>
                    <br>
                    <div class="btn-group">
                        <button class="">No</button>
                        <button class="">Fill</button>
                    </div>
                </div>
                <div class="col-6">
                    <pre><code class="HTML" data-lang="HTML">&lt;div class=&quot;btn-group btn-group-fill&quot;&gt;
    &lt;button class=&quot;btn-dark&quot;&gt;Look&lt;/button&gt;
    &lt;button class=&quot;btn-dark&quot;&gt;At&lt;/button&gt;
    &lt;button class=&quot;btn-dark&quot;&gt;Me&lt;/button&gt;
    &lt;button class=&quot;btn-dark&quot;&gt;Flex&lt;/button&gt;
&lt;/div&gt;
&lt;br&gt;
&lt;div class=&quot;btn-group btn-group-fill&quot;&gt;
    &lt;button class=&quot;&quot;&gt;First Button&lt;/button&gt;
    &lt;button class=&quot;&quot;&gt;Second Button&lt;/button&gt;
&lt;/div&gt;
&lt;br&gt;
&lt;div class=&quot;btn-group&quot;&gt;
    &lt;button class=&quot;&quot;&gt;No&lt;/button&gt;
    &lt;button class=&quot;&quot;&gt;Fill&lt;/button&gt;
&lt;/div&gt;</code></pre>
                </div>
            </div>
        </div>
    </section>

        <!-- Pagination -->
    <section class="padtop">
        <div class="content">
            <div class="divider"></div>
            <ul class="pagination no-bullets">
                <li class="pagination-item pagination-prev">
                    <a href="../animations" class="display-block">
                        <p class="pagination-item-subtitle">Previous</p>
                        <h5 class="light no-margin">Animations</h5>
                    </a>
                </li>
                <li class="pagination-item pagination-next">
                    <a href="../cards" class="display-block">
                        <p class="pagination-item-subtitle">Next</p>
                        <h5 class="light no-margin">Cards</h5>
                    </a>
                </li>
            </ul>
        </div>
    </section>

    <script async src="../../demo_res/js/smoothscroll.js"></script>
    <script async src="../../demo_res/js/app.js"></script>
    <script src="https://use.fontawesome.com/a696b91557.js"></script>

    <!-- Syntax highlight code -->
    <script src="https://cdnjs.cloudflare.com/ajax/libs/highlight.js/9.11.0/highlight.min.js"></script>
    <script>hljs.initHighlightingOnLoad();</script>
</body>

<footer>
        <h6 class="white uppercase">Cirrus</h6>
        <div class="content">
            <div class="divider"></div>
            <!--<li class="no-bullets footer-list-title">Docs</li>-->
            <div class="row">
                <div class="col-3">
                    <ul class="no-bullets">
                        <a href="#"><li class="list-item">Animations</li></a>
                        <a href="#"><li class="list-item">Buttons</li></a>
                        <a href="#"><li class="list-item">Cards</li></a>
                    <ul>
                </div>
                 <div class="col-3">
                    <ul class="no-bullets">
                        <a href="#"><li class="list-item">Font</li></a>
                        <a href="#"><li class="list-item">Footer</li></a>
                        <a href="#"><li class="list-item">Forms</li></a>
                    <ul>
                </div>
                <div class="col-3">
                    <ul class="no-bullets">
                        <a href="#"><li class="list-item">Header</li></a>
                        <a href="#"><li class="list-item">Layout</li></a>
                        <a href="#"><li class="list-item">Links</li></a>
                    <ul>
                </div>
                <div class="col-3">
                    <ul class="no-bullets">
                        <a href="#"><li class="list-item">Lists</li></a>
                        <a href="#"><li class="list-item">Table</li></a>
                        <a href="#"><li class="list-item">Util</li></a>
                    </ul>
                </div>
            </div>
        </div>
          <p class="subtitle">The source code is licensed <a href="http://opensource.org/licenses/mit-license.php">MIT</a>. The website content is licensed <a rel="license" href="http://creativecommons.org/licenses/by-nc-sa/4.0/">CC BY-NC-SA 4.0</a>.</p>
        <p>Made with <i class="fa fa-heart pulse" aria-hidden="true"></i> by <a href="http://stanleylim.me" target="_blank">Stanley Lim</a>
</p>
  </footer>

</html>